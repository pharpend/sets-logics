...
The problem here is that \xtb{base $n$ representations of real numbers
  are not unique}. That sounds like a contradiction, but it really
isn't. It turns out, and we might prove this later, that you can tweak
the rules of writing numbers down to make decimal representations
unique. For decimal (base $10$) numbers, we assert that the number
can't begin or end with an infinite string of $9$s.

\begin{ExerciseList}
  \Exercise{In base $8$ (octal) notation, how do we ensure that every number
    we write refers to a unique number?}
  \Exercise{In base $2$ (binary) notation, how do we ensure that every number
    we write refers to a unique number?}
  \Exercise{In base $2$ notation, how do we ensure that every number
    we write refers to a unique number?}
  \Exercise{In base $n$ notation, how do we ensure that every number
    we write refers to a unique number?}
